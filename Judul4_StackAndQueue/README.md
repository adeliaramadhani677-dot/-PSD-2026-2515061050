...
pertama saya menjalankan program lalu menampilkan judul "Pembongkaran Candi". Kemudian langsung muncul informasi status stack yaitu "Lapisan tercatat: 0/10" yang artinya stack masih kosong, belum ada satu pun lapisan batu yang saya catat. Di bawahnya tampil menu dengan 5 pilihan operasi yang bisa digunakan.
saya memilih angka 1 (Push) dan mengetik nama lapisan "a", program menjalankan fungsi push(). Di dalam fungsi itu, dicek apakah stack sudah penuh. Karena masih kosong, maka top_idx yang semula bernilai -1 ditambah 1 menjadi 0, lalu nama "a" saya simpan di posisi pertama array. Program lalu mencetak konfirmasi "Lapisan 'a' berhasil dicatat di posisi #1." Proses yang sama terus saya ulangi untuk lapisan b, c, d, e, f, g, h, i, dan j, setiap kali saya memilih Push dan memasukkan nama, posisi teratas stack naik satu angka dan nama lapisan tersimpan di slot berikutnya.
saya memasukkan lapisan ke-11 yaitu "k", program kembali masuk ke fungsi push() dan melakukan pengecekan is_full(). Karena top_idx sudah bernilai 9 yang berarti 10 slot telah terisi penuh, kondisi ini bernilai benar sehingga program tidak menyimpan data apapun dan langsung mencetak pesan "Stack penuh! Kapasitas catatan lapisan candi sudah maksimal." Lapisan "k" yang saya masukkan pun gagal tersimpan.
saya memilih angka 2 (Pop), program menjalankan fungsi pop(). Fungsi ini mengambil data pada posisi paling atas stack, yaitu "j" di posisi #10, lalu mencetak "Restorasi: 'j' diangkat kembali ke posisi semula." Setelah itu top_idx dikurangi 1 menjadi 8, sehingga slot #10 kembali kosong. data "j" tidak dihapus secara fisik dari array, tetapi dianggap tidak ada karena top_idx sudah tidak menunjuk ke sana lagi. Karena slot #10 kini kosong, saya berhasil memasukkan "k" pada percobaan berikutnya dan mendapat konfirmasi "Lapisan 'k' berhasil dicatat di posisi #10."
saya memilih angka 3 (Peek), program menjalankan fungsi peek() yang hanya membaca nilai di posisi top_idx tanpa mengubah apapun. Karena lapisan teratas saat itu adalah "k" di posisi #10, program mencetak "Lapisan teratas saat ini: 'k' (posisi #10)." Stack tetap utuh seperti sebelumnya, tidak ada data yang saya tambah maupun dikurangi.
saya memilih angka 4 (Tampilkan Semua), program menjalankan fungsi display() yang melakukan perulangan dari top_idx turun sampai 0. Hasilnya adalah seluruh isi stack ditampilkan dari posisi teratas ke terbawah, yaitu dari #10 k hingga #1 a. Di sebelah lapisan "k" terdapat keterangan "teratas" untuk menandai bahwa itulah posisi paling atas stack saat ini. 
saya memilih angka 5 (Keluar), kondisi perulangan while pilih != 5 menjadi tidak terpenuhi sehingga loop berhenti. Program kemudian mencetak "Program selesai. Pembongkaran candi tersimpan." sebagai tanda bahwa sesi yang saya lakukan telah berakhir.
